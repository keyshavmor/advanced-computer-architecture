...
skip_3:	
        ...
        l.d f8,sample+16($zero) # f8:sample[2]
	    ...
	    daddi $t6,$zero,1	# $t6: loop index counter, starts at 1
	    daddi $t7,$zero,1	# $t7: loop index counter, starts at 1
		...
		dsll $t0,$t7,3		# $t0 = 8 * i
        daddi $t2,$t2,-1      # N-1 Samples		
        

loop:   
        div.d f14,f13,f5	# f14 = weighted sum of samples * reciprocal	
        mul.d f9,f6,f21		# f9 = sample[i-1]*coeff[0] (8 cycles)
		mul.d f10,f7,f22	# f10 = sample[i]*coeff[1] (8 cycles)
		mov.d f6,f7		    # f6 = f7 (update of i-1)		
		mul.d f11,f8,f23	# f11 = sample[i+1]*coeff[2] (8 cycles)		
		...
		add.d f12,f9,f10	# f12 = f9 + f10	
        mov.d f7,f8		    # f7 = f8 (update of i)		
		l.d f8, sample+24($t0)	# f8 = new sample (update of i+1)	
		add.d f13,f11,f12	# f13 = f11 + f12 (weighted sum of samples)
		s.d f14, result($t0) 	# store in result[i]
		bne $t2,$t6,loop	# check if loop is over
		dsll $t0,$t6,3		# $t0 = 8 * i

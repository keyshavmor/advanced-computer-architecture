...
skip_2:
		...
		daddi $t5,$t1,-1	# $t5 = N_SAMPLES - 1 (branch delay slot)
...
loop:
		...
		mov.d f6,f7		# update of sample[i-1]
		mov.d f7,f8		# update of sample[i]	
		...
		s.d f14, result-8($t6) 	# store in result[i]
		bne $t6,$t5,loop	# check if we are done
		l.d f8, sample+8($t6)	# update of sample[i+1] (branch delay slot)
exit:		
		s.d f8, result($t6)	# result[N_SAMPLES-1] = samples[N_SAMPLES-1]

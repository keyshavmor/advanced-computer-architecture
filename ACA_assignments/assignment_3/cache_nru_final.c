	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case NRU :
{
struct cache_blk_t *blk_copy;
  blk_copy=cp->sets[set].way_head;
int i=0; 

 while ( i != rot_pointer)
{
blk_copy=blk_copy->way_next;
i++;
}
temp_rot_pointer=rot_pointer;
for(i= rot_pointer; i < (cp->assoc-1);i++)
{
  
 if(blk_copy->used_bit == 0)
...
if (blk_copy == cp->sets[set].way_tail )
{
blk_copy->way_next=cp->sets[set].way_head;
}
...
{
//rot_pointer = k;
if((temp_rot_pointer + k) >= (cp->assoc))
{
 rot_pointer = ((temp_rot_pointer + k) - (cp->assoc));
}
if((temp_rot_pointer + k) < (cp->assoc))
{
 rot_pointer = temp_rot_pointer + k;
}
...
if (cp->policy == NRU)
   	{
   		repl->used_bit = 1;
                ...
                struct cache_blk_t *blk_copy;
   		blk_copy=cp->sets[set].way_head;
   		for (blk_copy=cp->sets[set].way_head; blk_copy; blk_copy=blk_copy->way_next)
   		{
   			
   			if(blk_copy->used_bit == 1)
   			{
   				flag++;
   			}
   		}

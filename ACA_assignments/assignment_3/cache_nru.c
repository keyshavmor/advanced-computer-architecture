	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  int lat = 0;
  struct cache_blk_t *blk_copy, *blk_temp;
  unsigned int temp,fasthit_temp,flag=0,miss_flag=0;
  unsigned int used_bit_buffer[cp->assoc],used_bit_miss_buffer[cp->assoc];
  ...
  case NRU:
  {
           ...
           for( blk=cp->sets[set].way_head ; blk ; blk = blk->way_next )
           {
              ...
              if ( miss_flag == blk->rot_pointer )
              {

                  for (;blk;blk=blk->way_next)
                   {
                       miss_flag++;
                       if (blk->used_bit == 0)
                         {
                                repl = blk;
                                blk->rot_pointer = miss_flag;
                                if( blk->rot_pointer ==  ( cp->assoc-1) )
                                {
                                 blk->rot_pointer = 0;
                                }
                                miss_flag = 0 ;
  ...
  if ( cp->policy == NRU )
  {
      flag=0;
      repl->used_bit=1;
        for ( repl = cp->sets[set].way_head; repl = cp->sets[set].way_tail ; repl= repl->way_next)
        {
 ...
 if (temp < cp->hits )
 {
    ...
    temp =cp->hits;
    flag =0;
    if ( cp->policy == NRU && blk->used_bit == 1)
    {
          for ( blk=cp->sets[set].way_head; blk = cp->sets[set].way_tail ; blk=blk->way_next)
          {
